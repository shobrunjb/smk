...
class Timeanddate extends Component
{
    ...
    $year dalam 4 digit
    */
    public static function getMonthYearSortIndo($month, $year){
        return Timeanddate::getShortMonthIndo($month)." ".substr($year, 2,2);
    }
    ...
    public static function getDateTimeIndo2($datetime)
    {
        $item = explode(' ', $datetime);
        if (count($item) == 2) {
            return Timeanddate::getShortDateIndo($item[0]) . " - " . substr($item[1],0,5);
        } else {
            return "-";
        }
    }
    ...
    public static function getDateDiff($startdate, $endate){

        $tgl1 = strtotime($startdate); 
        $tgl2 = strtotime($endate); 
