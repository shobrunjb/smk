...
namespace common\uicomponent;

use backend\models\CpanelLeftmenu;
use backend\models\User;
use common\helpers\UsersAccess;
use Yii;
...
//        public static function getListLeftMenu($modelLeftMenu,$controler_curentpage,$action_curentpage,$controler,$countChildStatus=false){
        ...
        $labelHeader = 'Menu '; //.Yii::$app->user->identity->id;
        $level = Yii::$app->user->identity->user_level;
        if($level == "sales"){
          $labelHeader = "Outlet : ";
          $labelHeader .= User::getOutletPenjualan()->outlet->nama_outlet;
        }

        $leftMenu[] = ['label' => $labelHeader, 'options' => ['class' => 'header']];
        foreach ($datas as $data) {
            ...
            if ($access) {
                $menu = array();
                $menu["label"] = $data->value_indo;
                $menu["icon"] = $data->menu_icon;
                $menu["visible"] = true;
                $menu["url"] = [$data->url];

                $items = CpanelLeftmenuDB::getChildMenuList($data->id_leftmenu);
                $menu["items"] = $items;
		...
		$leftMenu[] = ['label' => 'Logout', 'icon' => 'sign-in', 'url' => ['site/logout'], 'visible' => true, 'template'=>'<a href="{url}" data-method="post"><i class="fa fa-sign-out"></i> {label}</a>'];
    ...
    public static function getChildMenuList($id_parent)
    {
        $lc = new CpanelLeftmenu;
        $datas = $lc::findAll([
            'id_parent_leftmenu' => $id_parent,
            'visible' => 1,
        ...
        foreach ($datas as $data) {
            ...
            if ($access) {
				$menu = array();
				//$menu["label"] = $data->auth." ".$data->value_indo;
				$menu["label"] = $data->value_indo;
				$menu["icon"] = $data->menu_icon;
				$menu["visible"] = true;
				$menu["url"] = [$data->url];
