...
function firstLoad(){
    ...
    //Get first note from bot
    $( "#msginfo" ).text( "Silakan Tunggu. Admin sedang menyapa anda!" );
    setTimeout(function (){
      var msgText = "server membalas";
    
      var newval = msgText.replace("/", "****");

      $.post("get-first-bot-response/", {msg: newval, is: "<?= $is?>", ipe: "<?= $id_pegawai_enc ?>"}, function(data, status){
