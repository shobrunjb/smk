...
//                'no_dossier',
	...
	foreach($datas as $data){
		$psyparameters = app\models\CcatGenericPsyParameter::find()
				->where(['id_ccat_jenis_ujian' => $data->id_ccat_jenis_ujian])
				->all();
		foreach($psyparameters as $sub){
			$listColumn[] =  
			[
				//'label' => $data->jenis_ujian."[".$sub->nama_parameter."]",
				'label' => $data->id_ccat_jenis_ujian." [".$sub->nama_parameter."]",
				'value' =>  function ($model, $key, $index) use ($data, $sub) {
					//return "[0]";
					//return $model->id_pegawai." ".$data->id_ccat_jenis_ujian." ".$sub->id_ccat_generic_psy_parameter;
					return app\models\CcatGenericPsyResultSearch::getValue($model->id_pegawai,
						$data->id_ccat_jenis_ujian,0, $sub->id_ccat_generic_psy_parameter);
				}
			];
		}
	}
