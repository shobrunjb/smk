...
<tr>
	<td>
		<input type="checkbox" value="click" name="checked_today_<?= $id ?>" id="checked_today_<?= $id ?>" class="pull-right" <?= $checked ?> onclick="displayChecked<?= $id ?>()"/>
	</td>	
	<td width="20px">#<?= $model_pb->order_number ?></td>	
	<td><i><?= $model_pb->product_backlog ?></i></td>


	<td><span class="badge bg-blue"><?= $progress ?>%</span></td>
	<td>

		<div class="slidecontainer" id="slidecontainer_<?= $id ?>" style="<?= $style ?>">
		  <input type="range" min="0" max="100" class="slider" id="my_progress_<?= $model_pb->id_product_backlog ?>"  name="my_progress_<?= $model_pb->id_product_backlog ?>" value="<?= $progress+$contribution ?>">
		  <p>Target Progres: <span id="result_<?= $id ?>"></span></p>
		</div>
		<Script>
		...
		<Script>
			function displayChecked<?= $id ?>() {
			  //alert('PBI<?= $id ?>');
			  var x = document.getElementById("checked_today_<?= $id ?>");
			  var data = x.checked ;
			  //alert(data);
			  if (x.checked == true) {
			  	//alert("Show");
	...
	<td>
		...
		<p>Target Kontribusi Anda: <span id="kontribusi_<?= $id ?>"><?= $contribution ?></span>%</p>
