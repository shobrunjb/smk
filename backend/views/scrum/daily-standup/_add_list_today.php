...
<div class="panel panel-success"><div class="panel-heading">    
    ...
    <h3 class="panel-title"><span class="fa fa-check"></span> Silakan isikan di bagian mana backlog yang terkait beserta target kerja anda</h3>
...
<tr>
    ...
    <th>Target Progres Hari ini</th>
    <th>Target Kontribusi</th>
</tr>
...
<?php
$urld = Url::toRoute(['/member-sprint/detail/', 'i' => $i, 't'=>$t,  'm'=>"add", 's'=>3]);
?>
<script>
  ...
  if(today != ""){
      ...
      <?php
        ...
        foreach($mods as $mod){
            ...
            //echo 'var x = $( "#checked_today_'.$id.'" );';
            echo 'var x = document.getElementById("checked_today_'.$id.'");';
            echo '
              if (x.checked == true) {
                //alert("cek'.$id.'");
                totalchecked++;
      ...
      */
      ...
      if(totalchecked > 0){
        ...
        $.post("<?= Yii::$app->request->baseUrl ?>/api/submit-daily-second-step/", {today: today, rowsdata: rowsdata, is : <?= $sprint->id_sprint;?>, ip : <?= $sprint->id_project;?>}, function(data, status){
            //alert("Saving : " + data + "\nStatus: " + status);
            //$( "#msginfo").text( data);
            $( "#msginfo").html( data);
        ...
        $( "#msginfo" ).html( "<div class='alert alert-danger'>Silakan isikan minimal 1 backlog yang terkait dengan pekerjaan anda hari ini beserta target yang akan dilakukan (dalam persen) !</div>" );
      }
    }else{
        $( "#msginfo" ).html( "<div class='alert alert-danger'>Silakan isikan terlebih dahulu rencana kegiatan ini anda di hari ini!</div>" );
    }
}
</script>