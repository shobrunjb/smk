...
switch ($action) {
    case "index":
        ...
        if(isset($_POST['saveall'])){
            //3. Disimpan atau diupdate di sprint close
            $sprintceremony = \backend\models\SprintCeremony::find()
                ->where([
                    'id_sprint' => $sprint->id_sprint,
                    'id_project' => $sprint->id_project,
        ...
        if(isset($_POST['lock'])){
            ...
            if($yourrole == $po_role){
                ...
                $sprint->is_closed = 2; //Ditutup jadi bernilai 2
                $sprint->is_locked = 2; //Dikunci jadi bernilai 2
                $sprint->save(false);

                Yii::$app->response->redirect(['member-sprint/detail/', 'i' => $i, 't' => $t]);
            }else{
                $messagelock = 'Untuk mengunci sprint close hanya bisa dilakukan oleh Product Owner!';
            }
        }
        ...
        if(isset($_POST['unlock'])){
            ...
            if($yourrole == $po_role){
                //$messagelock = 'Anda berhak mengunci dan membuka kunci lagi';
                $sprint->is_closed = 0; //Dikunci jadi bernilai 2
                $sprint->save(false);

                Yii::$app->response->redirect(['member-sprint/detail/', 'i' => $i, 't' => $t]);
            }else{
                $messagelock = 'Untuk membuka kuncil sprint close hanya bisa dilakukan oleh Product Owner!';
            }
        }

        echo  $this->render('/scrum/sprint-close/index', [
    ...
    case "index-readonly-old":
        ...
        echo  $this->render('/scrum/sprint-close/index-readonly', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
            'action' => $action,
            't' => $t,
    ...
    case "index-readonly":
        ...
        echo  $this->render('/scrum/sprint-close/index-sprint-backlog', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
            'action' => $action,
            't' => $t,
