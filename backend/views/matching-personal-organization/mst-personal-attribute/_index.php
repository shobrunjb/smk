...
$id_organization = $model->id_perusahaan;
//echo $id_organization."<==id organization";
?>
<div class="box">
    <div class="box-body mst-personal-attribute-index">
        <?= Html::beginForm(['personal-attribute/save-change', 'i' => $i], 'POST'); ?>
            ...
            'columns' => [
                    ...
                    [
                        ...
                        'format' => 'raw',
                        'value' => function ($data) use ($status_view, $id_organization, $id_pegawai) {
                            $result = '';
                            ...
                            if($lastvaluePersonal != null){
                                ...
                                if(isset($lastvaluePersonal->grade)){
                                    ...
                                    //Mencari ID attribute grade untuk organization
                                    $organizationAttributeGrade = 
                                    \backend\models\MstOrganizationAttributeGrade::find()
                                        ...
                                        ->one();
                                    if($organizationAttributeGrade != null){
                                        //$result .= "id2->".$organizationAttributeGrade->id_mst_organization_attribute_grade;
                                        ...
                                        \backend\models\ProOrganizationAttribute::find()
                                            ->where([
                                                ...
                                                //'id_mst_organization_attribute' => $data->id_mst_organization_attribute,
                                                'id_mst_organization_attribute_grade' =>$organizationAttributeGrade->id_mst_organization_attribute_grade
                                            ])
                                            ->one();
        ...
        <?php
            ...
            foreach($models as $data){
                ...
                if($data->personal_attribute == "kecocokan_perusahaan"){
            ...
            //Hasilnya di simpan di tabel MatchingOrganizationResult
            $result = \backend\models\MatchingOrganizationResult::find()
                ->where([
                    'id_perusahaan' => $id_organization,
                    'id_pegawai' =>$id_pegawai,
                ])
                ->one();
            if($result == null){
                $result = new  \backend\models\MatchingOrganizationResult();
                $result->id_perusahaan = $id_organization;
                $result->id_pegawai = $id_pegawai;
            }

            if($result->fit_value>=80){
                if($result->acknowledge_individul_fit_label=="Sangat Cocok / Sesuai"){                
                    $result->check = "Wajar";
                }else{
                    $result->check = "Tidak Wajar";
                }
            }else if($result->fit_value<80){
